...
pub fn spawn_engine() -> EngineHandle {
    ...
    thread::spawn(move || {
        ...
        loop {
            ...
            let is_playing_now = engine_playing.load(Ordering::Relaxed);
            
            // Handle pause state transition
            if was_playing && !is_playing_now {
                // We just paused, hardware reset
                for i in 1..=8 {
                    println!("{}:OFF", i);
                }
            }
            ...
            if is_playing_now {
                ...
                while current_queue_index < queue.len() {
                    ...
                    if action.time_ms <= current_time {
                        ...
                        println!("{}:{}", action.relay_id, state_str);
...
pub fn compile_timeline(timeline: &Timeline) -> Vec<CompiledAction> {
